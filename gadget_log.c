...
void gadget_log_help()
{
    xprint( NORMAL,
            "Create embedded Linux apps - easy.\n"
            "\n"
            "usage: gadget log [<project_path>]\n"
            "\n"
            "optional arguments:\n"
            "  -h, --help            show this help message and exit\n"
            "  -v, --verbose         be verbose\n"
            "  <project_path>        stop project in path (default: .) on device\n"
          );
}

int gadget_log(int argc,char **argv)
{
    int c, ret=0;
    ...
    if(!(project=gadget_project_deserialize("%s/%s",project_path, GADGET_CONFIG_FILE))) {
        xprint(ERROR,"gadget build: ERROR: cannot read project file: '%s/%s'\n",project_path, GADGET_CONFIG_FILE);
        goto _return;
    }
    
    do_rpc(ENDPOINT_APPLICATION_LOG,project);
